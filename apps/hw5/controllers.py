...
def get_posts():
    ...
    posts=db(db.post).select(db.post.ALL,orderby=~db.post.ts).as_list()
    formatted_post = []
    for post in posts:
        formatted_post.append(format_post(post["id"]))
    return dict(posts=formatted_post)
...
def delete_post():
    ...
    if post_id is not None and user_email==auth.current_user.get("email"):
