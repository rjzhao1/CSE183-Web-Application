...
let init = (app) => {
   ...
   app.refresh = () =>{
      let temp = app.vue.hover_post=null;
   ...
   app.user_thumb_on_post= (post,rating)=>{
    ...
    app.send_post = () =>{
         axios.post(add_post_url,{post_text:app.vue.post_text}).then((result)=> {
            ...
            app.vue.posts = app.reindex([result.data.post,...app.vue.posts]);
            app.clear_new_post();
         }).catch((e)=> alert("Error Sending Posts:",e));
   };
   ...
   app.get_posts = () => {
    axios.get(get_posts_url).then((result)=> {
      ...
      }).catch((e)=> alert("Error Getting Posts:",e));
   };
